...
RF_df_test = pd.read_csv(r'E:/ICHproject/(Radiomics+Clinical)_testing.csv')
...
incep_DF_train = pd.read_csv('DLR_IncepModel_train.csv',header=None)
incep_DF_test = pd.read_csv('DLR_IncepModel_test.csv', header=None)
...
'''
complex_train = result = pd.merge(lincep_DF_train, AlexNet_DF_train)
complcex_test = result = pd.merge(lincep_DF_test, AlexNet_DF_test)
...
#     plt.imshow(cm_normalized, interpolation='nearest', cmap=plt.cm.Blues)
    ...
    plt.plot(fpr, tpr, color='C1', lw=3,
             label='ROC curve of apple_pie (AUC = %0.2f)' % ROC_AUC)
...
X_train1, X_test1 = testing_model(
    radiomics_train, incep_DF_train, radiomics_test, incep_DF_test).concatenate()
X_train_1, y_train, X_test_1, y_test = processing(
...
gs, K_best = SFS_GS(X_train_p, y_trainp, X_test_p,
                    y_testp, model_SVC, param_grid_SVC, X_train_p)

##gs, pipe = SFS_GS(X_train_1, y_train, X_test_1, y_test, model_SVC, param_grid_SVC, X_train1)
#gs, pipe = SFS_GS(X_train_1, y_train, X_test_1, y_test,
#                 model_SVC, param_grid_SVC_nested_2, X_train1)
