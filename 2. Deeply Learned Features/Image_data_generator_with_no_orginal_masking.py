...
#test_dir = r'C:/Users/zy/Desktop/testing'
...
def get_dataset_long(search_dir):
    ...
    for x in range(len(dataset.patient_ids)):
        ...
        maska = sitk.GetArrayFromImage(mask_file)
        maskb = sitk.GetArrayFromImage(mask_file)
        maskc = sitk.GetArrayFromImage(mask_file)
        print('mask_array.shape', maska.shape)
        #show_img(mask)
...
#       mask1 = choose_channal(maska, channels-2)

#       mask2 = choose_channal(maskb, channels)
#       mask3 = choose_channal(maskc, channels+2)
        ...
        ROI1 = ROI1.reshape(512,512,1)
        ROI2 = ROI2.reshape(512,512,1)
        ROI3 = ROI3.reshape(512,512,1)
...
#       ROI1 = ROI_extraction(img, mask1, dataset.patient_ids[x])
#       print('ROI1')
...
#       print('ROI3')
        ...
        final1 = np.dstack((ROI1,))
        final2 = np.dstack((ROI2,))
...
train = get_dataset_long(train_dir)
#test = get_dataset_long(test_dir)


np.save('train_longformat_with_origianl.npy',train)
#np.save('test_longformat.npy',test)
...
train = np.load('train_longformat_with_origianl.npy')
...
np.save('y_train_long_with_original.npy',y_train_long)
